...
int read_info(const char *file_name,int *counter, int *mean_steps){
    ...
    *mean_steps = 0;
    ...
    for (count=0;count<*counter;count++){
        ...
        strcpy(temp_date,(fitdata[count].date));
        strcpy(temp_time,(fitdata[count].time));
        if (fitdata[count].steps<lowest_steps){
            ...
            strcpy(temp_date2,(fitdata[count].date));
            strcpy(temp_time2,fitdata[count].time);
        }
    }
    ...
    unrounded_mean = total_steps/(*counter);
...
int main() {
    ...
    while(1){
        ...
        switch (option){
                ...
                read_info(file_name,&counter,&mean_steps);
                break;
